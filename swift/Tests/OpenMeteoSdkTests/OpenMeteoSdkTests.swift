...
final class OpenMeteoSdkTests: XCTestCase {
    ...
    func testApiResponse() throws {
        ...
        let api = openmeteo_sdk_ApiResponse.createApiResponse(
            &fbb,
            latitude: 40,
            longitude: 50,
            elevation: 1200,
    ...
    func testSeries() throws {
        var fbb = FlatBufferBuilder()
        // Note: Offsets to data do not contain any data, but just ensure the offset is at the right position
        let api = openmeteo_sdk_Series.createSeries(
            &fbb,
        ...
        var bb = fbb.buffer
        let series = getRoot(byteBuffer: &bb) as openmeteo_sdk_Series
        XCTAssertEqual(series.ensembleMember, 50)
        XCTAssertEqual(series.unit, .celsius)
        XCTAssertEqual(series.variable, .temperature)
    }
}
